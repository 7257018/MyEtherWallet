<template>
  <form class="user-input">
    ...
    <p v-show="value.length > 0">{{ $t("cnSim.title") }} <span :class="strengthClass">{{ cnSim }}</span></p>
...
export default {
  ...
  methods: {
    async updateValue (value) {
      ...
      switch (score) {
        case 1:
          this.strength = this.$t('cnSim.veryWeak')
          this.strengthClass = 'very-weak'
          break
        case 2:
          this.strength = this.$t('cnSim.weak')
          this.strengthClass = 'weak'
          break
        case 3:
          this.strength = this.$t('cnSim.strong')
          this.strengthClass = 'strong'
          break
        case 4:
          this.strength = this.$t('cnSim.veryStrong')
